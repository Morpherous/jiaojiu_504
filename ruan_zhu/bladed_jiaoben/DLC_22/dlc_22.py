...
class DLC_22():
    ...
    def replace_file_by_parameters(self, file, compare_result_num, parameters):
        ...
        for i in range(len(compare_result_num)):
            ...
            if 'PTLOCK' in lines[index] and 'PTLOCK' in parameters:
                temp_line = parameters['PTLOCK']
            lines[index] = temp_line
        return lines

    def generate_batch(self):
        batch_num = 1
        csv_file = pd.read_csv(self.opt['DLC']['DLC_22']['Csv_Path'])
        pbar = tqdm(range(csv_file.shape[0]))

        try:
            os.makedirs(os.path.join(self.opt['Root_path'], 'batch/dlc2.2/'))
        except:
            pass

        batch_lst = open(os.path.join(self.opt['Root_path'], 'batch/dlc2.2/', 'batch.lst'), 'w+')
        batch_lst.write('VERSION	4.3.0.74\n')
        batch_lst.write('NUMBAT	%d\n' % csv_file.shape[0])
        for index in pbar:
            pbar.set_description('Generating Batch DLC2.2')
            # 写batch.x文件
            path = ('PATH\t%s\n' % os.path.join(self.opt['Root_path'], 'run/dlc2.2/',
            ...
            with open(os.path.join(self.opt['Root_path'], 'batch/dlc2.2/', 'Batch.' + str(batch_num)), 'w+') as f:
            ...
            with open(os.path.join(self.opt['Root_path'], 'batch/dlc2.2/', 'BatchPrj.' + str(batch_num)), 'w+') as ff:
                for line in lines_prj:
                    ff.write(str(line))

            batch_lst.write(
                'IFILE	Batch.%d\nIPATH\t%s\nIEXTN\tpowprod\nIEXEC\t4:dtbladed\nICALC	10\nISTST\t-1;0;-1;-1;0\nIENAB\t-1\nDISCON\t-\n' % (
                    batch_num, (os.path.join(self.opt['Root_path'], 'run/dlc2.2/',
                                             csv_file.iloc[index]['dlc_index'][3:])).replace('/', '\\')
                ))
            batch_num += 1
...
# 测试代码
# opt = CONF().parse_json()
# dlc11 = DLC_22(opt, '../DLC_22/compare_file/Batch.1', '../DLC_22/compare_file/Batch.2',
